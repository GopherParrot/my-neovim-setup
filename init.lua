...
vim.api.nvim_create_autocmd("FileType", {
  pattern = {"go", "c", "cpp", "lua", "txt"},
  callback = function()
    vim.opt_local.expandtab = false -- tabs, not spaces (Go style)
...
require('lazy').setup({
    ...
    -- The author explicitly recommends NOT using lazy loading.
    lazy = false,
    -- This key defines your keymaps, as is standard with lazy.nvim.
    keys = {
      -- Use the correct command with a space: Neominimap Toggle
      { "<leader>nm", "<cmd>Neominimap Toggle<cr>", desc = "Toggle Minimap" },
    },
    -- The 'init' function is what the author recommends for setup.
    -- It runs before the plugin is loaded.
    init = function()
      -- The author recommends setting a global variable for configuration.
      vim.g.neominimap = {
        auto_enable = false, -- Automatically enable the minimap
        -- You can add the other options you found here, for example:
        minimap_width = 20,
        use_lsp = true,
        use_treesitter = true,
        use_git = true,
        ...
        {
          "github/copilot.vim",
          -- No need for any special options or dependencies, it's all handled internally.
  ...
  -- Your main Copilot Chat plugin configuration
  {
    ...
    dependencies = {
      ...
      { "github/copilot.vim" }, -- Explicitly declare dependency
    },
    build = "make tiktoken",
    opts = {
    ...
    keys = {
      ...
      { "<leader>ca", "<cmd>CopilotChat<cr>", desc = "Ask Copilot" },
    ...
    config = function()
      ...
      require("notify").setup({
        stages = "fade_in_slide_out", -- "fade", "slide", "fade_in_slide_out", etc
        timeout = 3000,
        render = "default",
        background_colour = "#1e1e2e", -- Change if your terminal has issues
  ...
  -- bufferline.nvim
  -- NOTE: This is a plugin that provides a tab-like interface for managing buffers in Neovim.
  {
    ...
    config = function()
      require("bufferline").setup {
        options = {
            mode = "buffers", -- or "tabs" if you prefer tabpages
            numbers = "ordinal",
  ...
  -- lualine
  -- NOTE: This is a status line plugin that provides a customizable status line at the bottom of the screen.
  {
  ...
  -- auto session
  {
    ...
    config = function()
      require("auto-session").setup({
        log_level = "info",
        auto_restore_enabled = false, -- Automatically restores session when you open nvim in a directory
        auto_save_enabled = true,    -- Automatically saves session when you exit nvim
        auto_session_enable_last_session = false,
        auto_session_restore_by_dir = true,
        auto_session_use_git_branch = true,
        -- You can suppress certain directories from having sessions created
        -- auto_session_suppress_dirs = { "~/", "~/Projects", "~/Downloads", "/" },
        -- You can add hooks for pre/post restore/save actions
        -- post_restore_cmds = { "NeotreeShow" }, -- Example: if you use Neo-tree
      })
    end,
  },
  -- Optional: For Telescope integration (highly recommended)
  {
    "rmagatti/session-lens",
    dependencies = { "rmagatti/auto-session", "nvim-telescope/telescope.nvim" },
    config = function()
      require("session-lens").setup({
        -- Configure session-lens options here if needed
        -- For example, to change how paths are displayed:
        -- path_display = { "shorten" },
        -- To customize the Telescope picker theme:
  ...
  -- alpha nvim
  {
    "goolord/alpha-nvim",
    config = function()
      local ascii_arts = require("config.nvim_ascii")
      local num_arts = #ascii_arts
      local random_index = math.random(1, num_arts)
      local random_art = ascii_arts[random_index]

      local alpha = require("alpha")
      local dashboard = require("alpha.themes.dashboard")

      local TextConfig = require("config.text")
      local random_greeting = TextConfig.get_random_greeting()

      -- Helper function to find the max width of a multi-line string
      local function get_max_width(multi_line_string)
        local max_width = 0
        for line in string.gmatch(multi_line_string, "([^\n]*)\n?") do
          local line_width = #line -- This assumes standard ASCII characters (no wide characters)
      ...
      local function center_string(str, total_width)
        local len = #str
      ...
      -- Get the maximum width of the chosen ASCII art
      local art_max_width = get_max_width(random_art)
      -- Center the greeting text based on the ASCII art's max width
      local centered_greeting = center_string(random_greeting, art_max_width)

      -- Assign only the random ASCII art to the header section
      dashboard.section.header.val = random_art
      dashboard.section.header.opts.hl = "AlphaHeader"

      -- Create a new section for the centered greeting text
      dashboard.section.greeting_text = {
        type = "text",
        val = centered_greeting,
        opts = {
          hl = "Normal", -- Or "AlphaText" or any other highlight group you prefer
        },
        position = "center",
      }
      ...
      dashboard.section.footer.opts.hl = "Normal" -- Ensure footer is cleared and not causing weird highlight
  ...
  {
    "jxm35/livecode.nvim",
    -- It's good practice to set 'lazy = false' or provide 'cmd'
    -- If you want it always available, 'lazy = false' is fine.
    -- If you want it to load only when you use its commands (e.g., :LCShareBuffer),
    -- you can use 'cmd = { "LCShareBuffer", "LCJoin", "LCStop" }'
    -- For a simple setup, lazy = false might be easier to start with.
    lazy = false,
    config = function()
      require("livecode").setup({
        -- Optional: Set your username. This will be displayed to others in the session.
        username = "GopherParrot", -- NOTE: Replace with your desired username
        -- For example, if you want to use a different default port:
        -- port = 9000,
        -- Refer to :help livecode-nvim for all available options.
      })
    end,
  },
